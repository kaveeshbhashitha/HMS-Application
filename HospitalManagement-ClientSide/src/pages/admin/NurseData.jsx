...
export default function NurseData() {
    ...
    const searchById = async () => {
      try {
        const response = await axios.get('http://localhost:8080/nurseAppoint/searchAll');
    ...
    const handleDelete = async (suId) => {
      try {
        const response = await axios.delete(`http://localhost:8080/nurseAppoint/delete/${suId}`);
        if (response.status === 200) {
          setMessage({ text: 'Appointment for nurse was deleted successfully', color: 'green' });
          searchById();
        } else {
          setMessage({ text: 'Failed to cancel an delete Appointment', color: 'red' });
          searchById();
        }
      } catch (error) {
  ...
  return (
    <div>
        ...
        <form class="form-inline d-flex">
          ...
          <button type="submit" class="btn btn-primary mb-2 mx-1" onClick={handleNavigate}>Back To Appointments</button>
        </form>
        </nav>
          <div className='my-2 px-4'><h3>Nurse appointment data</h3></div>
            ...
            <table class="table table-hover">
              ...
              <tbody>
                {searchResult.map((user) => (
                  ...
                  <td>
                    <button className='btn btn-danger mx-1' onClick={() => handleDelete(user.naId)}>
